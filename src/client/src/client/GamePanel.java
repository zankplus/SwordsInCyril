...
class GamePanel extends JPanel
{
	...
	public ArrayList<Integer> getTargets(ZankMapTile selectedTile, FFTASkill sk, ActiveUnit user)
	{
		...
		{
			int dx = selectedTile.x - user.x, dy = selectedTile.y - user.y;
	...
	public void testSetup()
	{
		...
		ActiveUnit au3 = new ActiveUnit( (FFTAUnit) rosterPanel.roster.getModel().getElementAt(4), 1, 9, 7, 2);
					...
					for (int i = 0; i < hlTiles.size(); i++)
						if (selectedTile != null && selectedTile.x == hlTiles.get(i).xTile && selectedTile.y == hlTiles.get(i).yTile)
						{
							ArrayList<Integer> targetIDs = gamePanel.getTargets(selectedTile,
									gamePanel.selectedSkill, gamePanel.units[gamePanel.currentUnit]);
