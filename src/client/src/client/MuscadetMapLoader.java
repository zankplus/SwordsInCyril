...
public class MuscadetMapLoader
{
	...
	public static ZankGameMap getMap()
	{
		...
		map.mapData = new ZankMapTile[15][15];
		// map.tileList = new ZankMapTile ;
		...
		ZankMapTile[][] mapData = new ZankMapTile[15][15];
		...
		for (int i = 0; i < 15; i++)
			for (int j = 0; j < 15; j++)
				if (tiles[i][j] != 0)
				{
					mapData[i][j] = new ZankMapTile(i, j, tiles[i][j]);
					tileCount++;
		//			System.out.println(i + "," + j + ": adding tile at " + mapData[i][j].x + ", " + mapData[i][j].y);
				}
		
		
		ZankMapTile[] tileList = new ZankMapTile[tileCount];
		tileCount = 0;
		...
		map.p1StartingTiles = new ZankMapTile[] { mapData[0][10], mapData[1][10], mapData[2][10], mapData[1][11], mapData[2][11], mapData[0][12], mapData[1][12], mapData[2][12] };
		map.p2StartingTiles = new ZankMapTile[] { mapData[12][0], mapData[12][1], mapData[12][2], mapData[11][1], mapData[11][2], mapData[10][1], mapData[10][2] };
